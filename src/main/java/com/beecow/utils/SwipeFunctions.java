...
public class SwipeFunctions {
    ...
    public void Swipe(int startX, int startY, int endX, int endY, int duration) throws Exception {
        try {
            ...
            System.out.println(" ===== SWIPE =====> Screen Width x Height (" + screenWidth + "," + screenHeight + ")");
            System.out.println(" ===== SWIPE =====> Start coordinate: [" + actualStartX + "," + actualStartY + "], End coordinate: [" + actualEndX + "," + actualEndY + "]");
    ...
    public void swipeLeftToRightElement(WebElement el) {
        ...
        System.out.println(" ===== SWIPE =====> size: " + size + " - leftX: " + leftX + " - rightX: " + rightX + " - middleY: " + middleY);
        if (Utils.getInstance().isAndroidDevice()) {
            ((AndroidDriver) driver).swipe(leftX, middleY, rightX, middleY, 500);
        } else ((IOSDriver) driver).swipe(leftX, middleY, rightX, middleY, 500);
    }
    public void swipeRightToLeftElement(WebElement el) {
        ...
        System.out.println(" ===== SWIPE =====> size: " + size + " - leftX: " + leftX + " - rightX: " + rightX + " - middleY: " + middleY);
        if (Utils.getInstance().isAndroidDevice()) {
            ((AndroidDriver) driver).swipe(rightX, middleY, leftX, middleY, 500);
        } else ((IOSDriver) driver).swipe(rightX, middleY, leftX, middleY, 500);
    }
    public void swipingDownElementFromBottomToTop(WebElement el) {
        ...
        if (Utils.getInstance().isAndroidDevice()) {
            ((AndroidDriver) driver).swipe(centerX, bottomY, centerX, topY, 500);
        } else ((IOSDriver) driver).swipe(centerX, bottomY, centerX, topY, 500);
    }
    ...
    public void swipingDownFromElementToOtherElement(WebElement thisElement, WebElement elementBottom) {
        ...
        int centerX = thisElement.getLocation().getX() + thisElement.getSize().getWidth() / 2;
        System.out.println(" ===== SWIPE =====> topY " + elementTopY + " - bottomY " + bottomY + " - centerX " + centerX);
        if (Utils.getInstance().isAndroidDevice()) {
            ((AndroidDriver) driver).swipe(centerX, elementTopY, centerX, bottomY, 500);
        } else ((IOSDriver) driver).swipe(centerX, elementTopY, centerX, bottomY, 500);
    }

    public void SwipexPath(String textcontain) {
        do {
            try {
...
//
//    iOSSwipe("//*[@label='Gift Cards' and @visible='true']");
}